...
設定方式：
1. 至 https://myaccount.google.com/apppasswords 產生 App Password，存入環境變數：
   export SMTP_PASSWORD="你的16碼AppPassword"
2. 確認 SENDER_EMAIL 與 SMTP_USER 一致
3. 修改下方 CONFIG 的 REPORT_URL 和 RECIPIENTS（如需異動）

用法：
python send_report_email.py          # 自動偵測本週週次
python send_report_email.py --week W25  # 指定週次
...
CONFIG = {
    ...
    # 報告設定
    "REPORT_URL": "https://agent863.github.io/medify/report-2026-W25-Jun15-21.html",
    "REPORT_PASSWORD": "9053",

    # 收件人列表
    "RECIPIENTS": [
        "agent@iclarityvision.com",
    ],
...
def main():
    ...
    parser.add_argument("--week", default=None, help="指定週次，例如 W25（預設自動偵測上週）")
